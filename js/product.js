...
function validateProductData(data) {
    if (!data.name || !data.code || !data.category || !data.description || !data.price || !data.imageUrl) {
        ...
        <div class="card product-card h-100">
            ...
            <div class="card-body p-3">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <span class="product-price fw-bold">₹${product.price.toLocaleString ? product.price.toLocaleString() : product.price}</span>
                    <button class="btn btn-custom btn-sm" onclick="addToCart(${product.id}, '${product.name}', '${product.code}', '${product.description}', ${product.price}, '${product.imageUrl}')">Add</button>
                </div>
                ${adminButtons}
            </div>
        </div>
...
async function editProduct(productId) {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('productCode').value = `PRD${product.productId}`;
            document.getElementById('productCategory').value = getCategoryFromId(product.categoryId);
            document.getElementById('productDescription').value = product.description;
            document.getElementById('productPrice').value = product.price;
            document.getElementById('productImage').value = `https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?q=80&w=400&auto=format&fit=crop`;
